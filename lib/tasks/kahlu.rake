...
namespace :kahlu do
  ...
  task :setup do
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run Kahlu?')
      ...
      loop do
        ...
        env['DB_NAME'] = prompt.ask('Name of PostgreSQL database:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'kahlu_production'
          q.modify :strip
        end

        env['DB_USER'] = prompt.ask('Name of PostgreSQL user:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'kahlu'
          q.modify :strip
        end
      ...
      loop do
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "kik Social <noreply@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail to: send_to, subject: 'Test', body: 'kik Social SMTP configuration works!'
          mail.deliver
          break
        rescue StandardError => e
      ...
      if prompt.yes?('Save configuration?')
        ...
        File.write(Rails.root.join('.env.production'), "# Generated with kahlu:setup on #{Time.now.utc}\n\n" + env.each_pair.map { |key, value| "#{key}=#{value}" }.join("\n") + "\n")
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the Kahlu server.'
        prompt.say "\n"
