...
class Request
  ...
  def initialize(verb, url, **options)
    ...
    raise Kahlu::HostValidationError, 'Instance does not support hidden service connections' if block_hidden_service?
  ...
  def set_common_headers!
    ...
    @headers['User-Agent']      = Kahlu::Version.user_agent
  ...
  module ClientLimit
    def body_with_limit(limit = 1.megabyte)
      raise Kahlu::LengthValidationError if content_length.present? && content_length > limit
      ...
      while (chunk = readpartial)
        ...
        raise Kahlu::LengthValidationError if contents.bytesize > limit
      end
      ...
      def open(host, *args)
        ...
        Resolv::DNS.open do |dns|
          ...
          addresses.each do |address|
            begin
              raise Kahlu::HostValidationError if PrivateAddressCheck.private_address?(IPAddr.new(address.to_s))
